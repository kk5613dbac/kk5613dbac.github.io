...
function GetSelectedAreaType()
{
	...
	var select1 = document.getElementsByName("selectName1")[0];

	for(var i=0;i<=select1.options.length-1;i++){
		if(select1.options[i].selected){
			keyArray.push(select1.options[i].text);
		}
	}
...
function GetResultAreaType(xmldoc,keyArray,ex_xmldoc)
{
	...
	if(keyArray.length == 0){
		var flag_nosearch = true;
	}else{
		var flag_nosearch = false;	
	}
	...
	for(var i=0;i<=hinmeiNode.length-1;i++){
		flag = 0;
		for(var j=0;j<=keyArray.length-1;j++){
			if((hinmeiNode[i].textContent.indexOf(keyArray[j])==-1) || (flag_nosearch)){
				flag = -1;
				break;
			}
		}
		if(flag != -1){
			...
			resultArray[l].push(hinmeiNode[i].textContent);
			resultArray[l] = GetAdditionalInfo(l+1,resultArray[l],hinmeiNode[i],ex_xmldoc);
			l = l + 1;
		}
	}
...
function LinkEchoTest(clickedText)
{
	...
	var winObj = window.open('','','width=400');
	...
	winObj.resizeTo(350+6,areaInfo.length*(12+32+25+36+25)-(bossCnt)*(32+25+36+25)+15);
